			...
			publish:function(){
				uni.getStorage({
					...
					success: (res) => {
						...
						let publishData = {
							type:_self.content.type,
							id:_self.content.id,
							userName:res.data.userName,
							userImg:res.data.userImg,
							publishTime:new Date(),
							publishComment:_self.publishComment
						}
						console.log(publishData)
						uni.request({
							url:'http://47.97.213.36:7300/mock/5e145ffaf2ecc141ee032ab8/leader/publish',
							method:'POST',
							data:{
								data:publishData
							},
							success: (res) => {
								...
								if(res.data.flag === 1){
									...
									_self.content.commentList = res.data.newCommomList
									uni.showToast({
										title:'发表成功'
									})
									uni.hideKeyboard()
